...
if (m.text.includes(tt)) {
                ...
                var url = tt
                try {
                    tk.tiktokdownload(url).then ( data => {
                        bob.sendMessage(m.chat, {video: {url:  data.nowm}})
                    } )
                } catch (e) {
...
🌟━━━━━━━━━🌟
                    ...
                    case 'ai':{
                        ...
                        const { key } = await bob.sendMessage(m.chat, {text: 'Jojo lagi ngetik'}, { quoted: m });
                        next.gemini(q).then ( data => {
                            bob.sendMessage(m.chat, { text: data.message, edit: key})
                        })
                    }
                    break
                    case 'chatbot':{
...
fakereply(rules)
                ...
                if (isLimit(m.sender, isCreator, isPremium, limitCount, limit)) return reply (`Poin kamu sudah habis silahkan kirim ${prefix}poin untuk mengecek Point Yang Tersedia`)
                    ...
                    limitAdd(sender, limit)
                    reply(mess.wait)
                    tk.tiktokdownload(args[0]).then ( data => {
                        bob.sendMessage(m.chat, {video: {url:  data.nowm}, caption: `Sukses`})
                    } )
                    }
                    break
